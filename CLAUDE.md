...
| `open_data_sources.md` | Catalog of publicly available datasets for AITYA's analytics: invoice-level payment prediction data (Kaggle), Greek-specific sources (Diavgeia, Bank of Greece, ATHEX, GEMI, data.gov.gr), European/US company financials (ESEF, Companies House, SEC EDGAR), payment behavior benchmarks (EU Payment Observatory, Intrum, Atradius, Hackett), macroeconomic APIs (ECB, Eurostat, FRED), public procurement (TED), credit/insolvency risk sources, bankruptcy prediction datasets, and synthetic data tools; includes bulk collection methods (APIs, bulk downloads) and a prioritized collection sequence |
| `energy_data_sources.md` | Catalog of publicly available datasets and APIs for energy sector (electricity, gas, renewables) decision support: ENTSO-E electricity markets, ENTSOG gas transmission, GIE storage/LNG, PVGIS/NASA POWER solar, ERA5/Open-Meteo weather, EU ETS carbon, grid infrastructure registries, commodity prices, and Greek-specific sources (ADMIE/IPTO, HEnEx, DESFA, RAAEY, DAPEEP); includes Python package summary (13 packages), open-source energy models (PyPSA, Atlite, pvlib), Kaggle datasets, and a quick-start stack for Greek energy analytics |
