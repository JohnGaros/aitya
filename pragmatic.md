...
- Stability: **Very High.** Clearing is one of the most rigid processes in SAP FI -- an item is either open or cleared, with no ambiguity. The clearing mechanism is a deterministic state machine, not a subjective status field.
- Key fields: `IsCleared`, `ClearingDate`, `ClearingDocument`, `NetDueDate`, `CashDiscount1Days`, `CashDiscount1Amount`, `AccountType` (D=Customer, K=Vendor), `CompanyCode`.
...
- Payment punctuality analysis: the delta between `NetDueDate` and `ClearingDate` is the most direct measure of actual vs. promised payment behavior at the invoice level.
...
The clearing fields in `I_OperationalAcctgDocItem` are equally portable. `IsCleared`, `ClearingDate`, and `NetDueDate` have identical semantics across every S/4HANA tenant. The clearing process itself is structurally invariant: an open item transitions to cleared when a payment or credit memo is matched against it, regardless of the company's industry or configuration. Features derived from clearing behavior -- days-past-due at clearing, discount utilization rate, clearing velocity by customer segment -- require no per-tenant customization. Payment terms codes (`ZTERM` values) vary per tenant, but the configuration layer reads `T052` at deployment to automatically resolve every code to canonical day counts and discount percentages, eliminating the need for manual mapping.
...
| Can we trust the numbers? | -- | Clearing is deterministic (open → cleared) with no subjective state | Reconciliation equation validates data integrity before model training |
...
- **Payment behavior prediction:** The `NetDueDate` vs. `ClearingDate` delta from `I_OperationalAcctgDocItem` is the single most powerful feature for predicting future payment behavior. Combined with dunning progression, it distinguishes customers who pay late-but-reliably from those who are genuinely deteriorating.
...
- A single read of `T052` at onboarding resolves every payment terms code to a canonical representation (e.g., "Net 30 days, 2% discount if paid within 10 days") with no human interpretation required.
